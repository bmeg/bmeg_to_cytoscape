...
def init():
    schema = requests.get('https://bmeg.io/meta/schema.json').json()
    schema = validate(schema)

    # setup credentials
    bmeg_url = 'https://bmeg.io/api'
...
def create_node_labels(edge_labels):
...
if not os.path.exists('node_labels.json'):
    create_node_labels(schema, grip)
